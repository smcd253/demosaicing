...
/*
function uniform() returns and unsigned char value indicating the average pixel value surrounding
location i,j over an NXN matrix
@param unsigned char (***sourceImageData) is the source image data
@param i,j is the location of the current pixel
@param N is the size of the filter matrix
*/
unsigned char uniform(unsigned char ***sourceImageData, int height, int width, int i, int j, int N)
{
	// properly refer to 3d array by ref: *(((unsigned char *)sourceImageData + (i)*width) + (j));
	float H;
	...
	for (int nh = -(N-1)/2; nh <= (N-1)/2; nh++)
	{
		...
		{
			H += (float)*(((unsigned char *)sourceImageData + (i+nh)*width) + (j+nw));
		}
	}
	H /= (float)(N*N);
...
int main(int argc, char *argv[])
{
	...
	for (int i = 0; i < height; i++)
	{
		...
		{
			destImageData[i][j][0] = uniform((unsigned char ***)sourceImageData, height, width, i, j, N);
		}
	}
